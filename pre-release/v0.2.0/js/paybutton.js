...
if (!document.getElementById(cssButtonId)) {
  ...
  link.rel = 'stylesheet';
  link.type = 'text/css';
  link.href = 'https://paybutton.org/pre-release/v0.1/css/buttons.css';
  link.id = cssButtonId;
  link.media = 'all';
...
if (!document.getElementById(cssModalId)) {
  ...
  link.rel = 'stylesheet';
  link.type = 'text/css';
  link.href = 'https://paybutton.org/pre-release/v0.1/css/modal.css';
  link.id = cssModalId;
  link.media = 'all';
  head.appendChild(link);
}
...
if (!document.getElementById(qrId)) {
  ...
  script.src = 'https://paybutton.org/pre-release/v0.1/js/qrjs2.js';
  script.id = qrId;
...
function playAudio() {
  var successAudio = new Audio(
    'https://paybutton.org/pre-release/v0.1/audio/pbding.mp3'
  );
...
function openModal(pbAttr) {
  ...
  var pbContent =
    ...
    qrImage +
    '" />' +
    '<img class="qricon" src="https://paybutton.org/images/bitcoincash_bare_logo.png" />' +
    ...
    '<div><span><a href="https://paybutton.org" target="_blank" style="color: orangeRed; text-decoration: none;">Powered by PayButton.org</a></span></div>' +
    '</div>' +
    '</div>' +
    '</div>';
...
var Paybutton = {
  render: function(id, config) {
    ...
    if (config.class) {
      ...
      newBtn.setAttribute('class', 'pay-button pbtnjs');
    }
